...
def get_config_dict():
    config_dict = {}
    config_dict["training_set_end_index"] = 142932
    config_dict["validation_set_end_index"] = 157932
...
model = GINSubgraphModel(input_dim=hidden_channels_in_network, hidden_dim=hidden_channels_in_network)
model.to(device)
...
string_for_files = "Graph2Cone_baseline"
...
def loss_file_saver(loss_file_path, val_losses, train_losses):
...
def eval():
    ...
    v_loss = []
    with torch.no_grad():
        for q2, q1, negative_samples, index_for_image_val, data_idx in tqdm(val_loader, total=len(val_loader)):
            q1 = q1.to(device)
            q2 = q2.to(device)
            negative_samples = [negative_sample.to(device) for negative_sample, _ in negative_samples]
            pos_out = model(q2, q1)
            pos_label = torch.ones_like(pos_out).to(device)
            pos_loss = criterion(pos_out, pos_label)

            neg_loss = 0
            for neg_graph in negative_samples:
                neg_out = model(q2, neg_graph)
                neg_label = torch.zeros_like(neg_out).to(device)
                neg_loss += criterion(neg_out, neg_label)

            neg_loss /= len(negative_samples)
...
def train(index_for_captions):
    ...
    model.train()
    t_loss = []
    for q2, q1, negative_samples, index_for_image_val, data_idx in tqdm(train_loader, total=len(train_loader)):
        optimizer.zero_grad()
        q1 = q1.to(device)
        q2 = q2.to(device)
        negative_samples = [negative_sample.to(device) for negative_sample, _ in negative_samples]
...
if __name__=="__main__":
    ...
    if not os.path.exists('Graph2Cone_baseline_saved_files'):
        os.makedirs('Graph2Cone_baseline_saved_files')
    full_model_path = "Graph2Cone_baseline_saved_files/" + string_for_files + "_full_model_" + timestamp + ".pt"
    statistics_file_path = "Graph2Cone_baseline_saved_files/" + string_for_files + "_statistics_" + timestamp + ".txt"
    loss_file_path = "Graph2Cone_baseline_saved_files/" + string_for_files + "_loss_file_" + timestamp + ".txt"
    loss_curve_path = "Graph2Cone_baseline_saved_files/" + string_for_files + "_loss_plot_" + timestamp + ".png"
    config_dict_file_path = "Graph2Cone_baseline_saved_files/" + string_for_files + "_config_dict_" + timestamp + ".txt"
